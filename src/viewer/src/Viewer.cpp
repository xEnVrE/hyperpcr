...
Viewer::Viewer(const ResourceFinder& resource_finder)
{
    ...
    if (show_completed)
    {
        const Bottle& completed_bottle = resource_finder.findGroup("COMPLETED");
        if (completed_bottle.isNull())
            throw(std::runtime_error(log_name_ + "::ctor. Malformed configuration file: cannot find COMPLETED section."));
        auto color = load_vector_uchar(completed_bottle, "color", 3);
        auto reference_frame_length = completed_bottle.check("reference_frame_length", Value(0.1)).asFloat64();
